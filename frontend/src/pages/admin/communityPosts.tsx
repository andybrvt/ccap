            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {posts.map((post) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div
                      className="w-9 h-9 rounded-full bg-secondary border border-line flex items-center justify-center text-ink text-sm font-semibold cursor-pointer hover:border-brand transition-colors flex-shrink-0"
            ...
            <div className="flex flex-col md:flex-row h-full">
              ...
              <div className="md:w-2/5 flex flex-col bg-white">
                <div className="p-4 border-b border-line flex items-center gap-3">
                  <div
                    className="w-10 h-10 rounded-full bg-secondary border border-line flex items-center justify-center text-ink font-semibold cursor-pointer hover:border-brand transition-colors"
                    onClick={(e) => selectedPost.author?.id && handleNavigateToPortfolio(selectedPost.author.id, e)}
                  >
                    {selectedPost.author?.username?.substring(0, 2).toUpperCase() || 'ST'}
                  </div>
                  <div className="flex-1">
                    <span
                      className="font-medium text-ink block cursor-pointer hover:underline"
                      onClick={(e) => selectedPost.author?.id && handleNavigateToPortfolio(selectedPost.author.id, e)}
                    >
